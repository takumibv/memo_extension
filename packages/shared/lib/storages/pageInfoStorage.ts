import { getNewId, getStorage, setStorage } from './common.js';
import { formURL } from '../utils/index.js';
...
const setPageInfoStorage = async (pageInfos: PageInfo[]): Promise<boolean> =>
  await setStorage(PAGE_INFO_STORAGE_NAME, pageInfos);
...
export const getAllPageInfos = async (): Promise<PageInfo[]> => await getPageInfoStorage();

export const getPageInfoById = async (): Promise<PageInfo> =>
  // TODO: implement this function with pageId parameter
  new Promise((_resolve, reject) => reject('getPageInfoById is not implemented'));
